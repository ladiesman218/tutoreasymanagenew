...
struct UserController: RouteCollection {
	...
	func boot(routes: RoutesBuilder) throws {
		...
		tokenAuthGroup.on(.POST, "profile-pic", body: .collect(maxSize: "10mb") , use: addProfilePicturePostHandler)
	}
...
extension UserController {
	...
	func addProfilePicturePostHandler(_ req: Request) async throws -> Response {
		let user = try req.auth.require(User.self)
		guard let data = try? req.content.decode(ImageUploadData.self) else {
	...
	func getUsersProfilePictureHandler(_ req: Request) async throws -> Response {
		...
		let user = try req.auth.require(User.self)
		guard let filename = user.profilePic else {
			throw Abort(.notFound)
		}
